...
import type { DirectoryPerson, OrgNode } from "@/lib/orgchart";
...
export function OrgNodeDialog({
  chartId, node, parentId, parentName, people, trigger,
}: {
  chartId: string;
  node?: OrgNode;
  ...
  useEffect(() => {
    ...
    if (node) {
      ...
      setPerson(null); setName(""); setRole("");
    }
  }, [open, node, people]);

  const save = useMutation({
    mutationFn: async () => {
      const finalName = (person?.name ?? name).trim();
      if (!finalName) throw new Error("Choose a person or type a name.");
      const payload = {
        chart_id: chartId,
        ...
        role_title: (role.trim() || person?.subtitle || null),
      };
      if (node) {
    ...
    onSuccess: () => {
      toast.success(node ? "Position updated" : "Position added");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{node ? "Edit position" : "Add position"}</DialogTitle>
          <DialogDescription>
